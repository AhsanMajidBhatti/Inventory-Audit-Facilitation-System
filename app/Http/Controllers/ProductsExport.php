...
class ProductsExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        // $dept = Depreciation::select(DB::raw("ARRAY_AGG(cost) as costing"))->groupBy('product_id')->sortBy('id', 'asc')->get()
        //     ->map(function ($attendances) {
        //         // $costs = explode(',', $attendances->costing);
        //         // return $costs;
        //         print_r($attendances->costing . "\n");
        //     });


        // $dept = Depreciation::select(DB::raw("array_to_string(ARRAY_AGG(product_id) ORDER BY id DESC) as costing"))->groupBy('product_id')->get()
        //     ->map(function ($attendances) {
        //         print_r($attendances->costing . "\n");
        //     });

        $product = Product::all();
        $product->makeHidden(['id', 'status', 'depreciation', 'repair_date', 'inactive_date', 'created_at', 'updated_at']);
    ...
    public function headings(): array
    {
        ...
        // $dept = Depreciation::select(DB::raw("STRING_AGG(name,',') as naming"))->distinct()->get()
        //     ->map(function ($attendances) {
        //         $names = explode(',', $attendances->naming);
        //         return $names;
        //     });
